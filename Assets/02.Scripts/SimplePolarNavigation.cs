...
public class SimplePolarNavigation : MonoBehaviour {
	...
	float m_max_phi = Mathf.PI / 2f * 0.9f;
	float m_min_phi =  Mathf.PI / 2f * 0.1f;
	...
	public void Set(Vector3 pivot, Vector3 camera) {
		...
		if(m_polar_target.phi < m_min_phi) m_polar_target.phi = m_min_phi;
	...
	public void RotateVertical(float rad) {
		...
		if(m_polar_target.phi > m_max_phi)
			m_polar_target.phi = m_max_phi;
		if(m_polar_target.phi < m_min_phi)
			m_polar_target.phi = m_min_phi;
	}
